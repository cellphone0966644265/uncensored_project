...
def create_project_structure():
    ...
    dirs_to_create = [
        ...
        "tool", "script_AI/run", "script_AI/train",
    ]
...
def handle_video(args, temp_dir):
    ...
    for i, chunk_path in enumerate(chunk_paths):
        ...
        if frames_info.get('audio_path'):
            proc_chunk_cmd.extend(["--audio_path", frames_info['audio_path']])
...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument('--input',required=True); a=parser.parse_args()
    video_ext = ['.mp4','.avi','.mov']; image_ext = ['.jpg','.jpeg','.png']
...
class AddYouknowModel(nn.Module):
    ...
    def forward(self, x): return F.interpolate(self.decoder(self.backbone(x)), size=x.shape[2:], mode='bilinear', align_corners=False)
class PConv(nn.Module):
    def __init__(self, in_ch, out_ch, bn=True, sample='none', activ='relu', bias=False):
        ...
        self.conv=nn.Conv2d(in_ch,out_ch,3,1,1,bias=bias); self.mask_conv=nn.Conv2d(in_ch,out_ch,3,1,1,bias=False); torch.nn.init.constant_(self.mask_conv.weight,1.0)
        for p in self.mask_conv.parameters(): p.requires_grad=False
        self.bn=nn.BatchNorm2d(out_ch) if bn else None; self.activ=nn.ReLU(True) if activ=='relu' else nn.LeakyReLU(0.2,True)
