...
def handle_video(args, temp_dir):
    ...
    for i, chunk_path in enumerate(chunk_paths):
        ...
        if args.task_name == 'add_youknow':
            ...
            run_command(["python", os.path.join(SCRIPT_AI_RUN_DIR, "run_add_youknow.py"), "--input_dir", frames_folder, "--output_dir", processed_frames_dir] + common_args)
        elif args.task_name == 'clean_youknow':
            mask_dir = os.path.join(chunk_proc_dir, "masks")
            processed_frames_dir = os.path.join(chunk_proc_dir, "processed")
            run_command(["python", os.path.join(SCRIPT_AI_RUN_DIR, "run_mosaic_position.py"), "--input_dir", frames_folder, "--output_dir", mask_dir] + common_args)
            run_command(["python", os.path.join(SCRIPT_AI_RUN_DIR, "run_clean_youknow.py"), "--input_dir", frames_folder, "--mask_dir", mask_dir, "--output_dir", processed_frames_dir] + common_args)
...
def main():
    ...
    try:
        ...
        print(json.dumps({'metadata':{'fps':fps, 'width':int(cap.get(3)), 'height':int(cap.get(4)), 'frame_count':int(cap.get(7)), 'duration_seconds':cap.get(7)/fps if fps>0 else 0}}))
    finally: cap.release()
if __name__ == "__main__": main()"""),
...
def main():
    ...
    temp_frame = os.path.join(temp_dir, f"temp_frame_{os.path.basename(args.input)}.png")
...
def main():
    ...
    ff_frames.run(stderr=sys.stderr); aop = None
    try: ff_audio.run(stderr=sys.stderr); aop = os.path.abspath(audio_path)
    except: pass
...
def main():
    ...
    fps = json.loads(args.metadata_json).get('fps'); os.makedirs(os.path.dirname(args.output), exist_ok=True)
    pattern = os.path.join(args.frame_folder, '%08d.png'); inputs = {pattern: ['-framerate', str(fps)]}
    ...
    ff.run(stderr=sys.stderr); print(json.dumps({'output_path':os.path.abspath(args.output)}))
if __name__ == "__main__": main()"""),
...
def main():
    ...
    ff.run(stderr=sys.stderr); print(json.dumps({'final_video_path':os.path.abspath(args.output)}))
if __name__ == "__main__": main()"""),
    # --- script_AI/ ---
    ("script_AI/images_processing.py", """import torch,cv2,numpy as np;from torchvision import transforms;
def load_image_to_tensor(p,s=(512,512),n=True):i=cv2.imread(p);o=i.shape[:2];i=cv2.cvtColor(i,cv2.COLOR_BGR2RGB);tl=[transforms.ToTensor(),transforms.Resize(s,antialias=True)];(lambda:tl.append(transforms.Normalize(mean=[.485,.456,.406],std=[.229,.224,.225])))[n]();return transforms.Compose(tl)(i).unsqueeze(0),o
def save_tensor_as_image(t,op,o,i=False):t=t.detach().cpu().squeeze(0);t=transforms.Resize(o,antialias=True)(t);if i:t=t*.5+.5;img=np.clip(t.permute(1,2,0).numpy()*255,0,255).astype(np.uint8);cv2.imwrite(op,cv2.cvtColor(img,cv2.COLOR_RGB2BGR))
def load_mask_to_tensor(p,s=(512,512)):m=cv2.imread(p,cv2.IMREAD_GRAYSCALE);_,m=cv2.threshold(m,127,255,cv2.THRESH_BINARY);return transforms.Compose([transforms.ToTensor(),transforms.Resize(s,antialias=True)])(m).unsqueeze(0)"""),
    ("script_AI/model_loader.py", """import torch,os,sys; SCRIPT_DIR=os.path.dirname(os.path.abspath(__file__)); sys.path.append(os.path.dirname(SCRIPT_DIR)); from script_AI.models import AddYouknowModel,CleanYouknowModel
def load_model(name,pth,dev='cpu'):
    ...
    sd=torch.load(pth,map_location=torch.device(dev));sd=sd.get('state_dict',sd)
...
class ResNet18(nn.Module):
    def __init__(s):super().__init__();r=torch.hub.load('pytorch/vision:v0.10.0','resnet18',True);s.f=nn.Sequential(*list(r.children())[:-2])
    def forward(s,x):return s.f(x)
class AddYouknowModel(nn.Module):
    def __init__(s,n=1):super().__init__();s.b=ResNet18();s.d=nn.Sequential(nn.Conv2d(512,256,3,padding=1),nn.ReLU(True),nn.Upsample(scale_factor=2),nn.Conv2d(256,128,3,padding=1),nn.ReLU(True),nn.Upsample(scale_factor=2),nn.Conv2d(128,64,3,padding=1),nn.ReLU(True),nn.Upsample(scale_factor=2),nn.Conv2d(64,32,3,padding=1),nn.ReLU(True),nn.Upsample(scale_factor=2),nn.Conv2d(32,n,3,padding=1))
    def forward(s,x):return F.interpolate(s.d(s.b(x)),size=x.shape[2:],mode='bilinear',align_corners=False)
class PConv(nn.Module):
    def __init__(s,i,o,b=True,p='none',a='relu',c=False):super().__init__();s.p=None;s.c=nn.Conv2d(i,o,3,1,1,bias=c);s.m=nn.Conv2d(i,o,3,1,1,bias=False);torch.nn.init.constant_(s.m.weight,1.0);s.b=nn.BatchNorm2d(o)if b else None;s.a=nn.ReLU(True)if a=='relu'else nn.LeakyReLU(.2,True);(lambda:setattr(s,'p',nn.MaxPool2d(2) if p=='down' else nn.Upsample(scale_factor=2,mode='bilinear',align_corners=True)))[p!='none']();[p.requires_grad(False)for p in s.m.parameters()]
    def forward(s,x,m):
        if s.p:x,m=s.p(x),s.p(m)
        with torch.no_grad():mr=1/(s.m(m)+1e-8)
        o=s.c(x*m)*mr;
        if s.b:o=s.b(o)
        return s.a(o),m
class CleanYouknowModel(nn.Module):
    def __init__(s):super().__init__();s.e1=PConv(4,64,bn=False,sample='down');s.e2=PConv(64,128,sample='down');s.e3=PConv(128,256,sample='down');s.e4=PConv(256,512,sample='down');s.d1=PConv(512+256,256,a='leaky',sample='up');s.d2=PConv(256+128,128,a='leaky',sample='up');s.d3=PConv(128+64,64,a='leaky',sample='up');s.d4=PConv(64+4,3,bn=False,a='leaky');s.f=nn.Conv2d(3,3,1)
    def forward(s,x,m):e1,m1=s.e1(x,m);e2,m2=s.e2(e1,m1);e3,m3=s.e3(e2,m2);e4,m4=s.e4(e3,m3);d1,_=s.d1(torch.cat([e4,e3],1),torch.cat([m4,m3],1));d2,_=s.d2(torch.cat([d1,e2],1),torch.cat([m2,m2],1));d3,_=s.d3(torch.cat([d2,e1],1),torch.cat([m2,m1],1));d4,_=s.d4(torch.cat([d3,x],1),torch.cat([m1,m],1));return torch.tanh(s.f(d4))"""),
    ("script_AI/run/run_add_youknow.py", "import argparse,json,sys,os,torch;from tqdm import tqdm;sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))));from script_AI.model_loader import load_model;from script_AI.images_processing import load_image_to_tensor,save_tensor_as_image;p=argparse.ArgumentParser();p.add_argument('--input_dir',required=True);p.add_argument('--output_dir',required=True);p.add_argument('--project_root',required=True);a=p.parse_args();d='cuda'if torch.cuda.is_available()else'cpu';m=load_model('add_youknow',os.path.join(a.project_root,'pre_trained_models/add_youknow.pth'),d);os.makedirs(a.output_dir,exist_ok=True);[save_tensor_as_image(torch.sigmoid(m(t.to(d))),os.path.join(a.output_dir,f),sz)for f in tqdm(os.listdir(a.input_dir),file=sys.stderr)for t,sz in[(load_image_to_tensor(os.path.join(a.input_dir,f)))]];print(json.dumps({'processed_frame_folder':os.path.abspath(a.output_dir)}))"),
    ("script_AI/run/run_mosaic_position.py", "import argparse,json,sys,os,torch;from tqdm import tqdm;sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))));from script_AI.model_loader import load_model;from script_AI.images_processing import load_image_to_tensor,save_tensor_as_image;p=argparse.ArgumentParser();p.add_argument('--input_dir',required=True);p.add_argument('--output_dir',required=True);p.add_argument('--project_root',required=True);a=p.parse_args();d='cuda'if torch.cuda.is_available()else'cpu';m=load_model('mosaic_position',os.path.join(a.project_root,'pre_trained_models/mosaic_position.pth'),d);os.makedirs(a.output_dir,exist_ok=True);[save_tensor_as_image(torch.sigmoid(m(t.to(d))),os.path.join(a.output_dir,f),sz)for f in tqdm(os.listdir(a.input_dir),file=sys.stderr)for t,sz in[(load_image_to_tensor(os.path.join(a.input_dir,f)))]];print(json.dumps({'processed_frame_folder':os.path.abspath(a.output_dir)}))"),
    ("script_AI/run/run_clean_youknow.py", "import argparse,json,sys,os,torch;from tqdm import tqdm;sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))));from script_AI.model_loader import load_model;from script_AI.images_processing import load_image_to_tensor,load_mask_to_tensor,save_tensor_as_image;p=argparse.ArgumentParser();p.add_argument('--input_dir',required=True);p.add_argument('--mask_dir',required=True);p.add_argument('--output_dir',required=True);p.add_argument('--project_root',required=True);a=p.parse_args();d='cuda'if torch.cuda.is_available()else'cpu';m=load_model('clean_youknow',os.path.join(a.project_root,'pre_trained_models/clean_youknow.pth'),d);os.makedirs(a.output_dir,exist_ok=True);[save_tensor_as_image((o*mt)+(it_n*(1-mt)),os.path.join(a.output_dir,f),sz,True)for f in tqdm(os.listdir(a.input_dir),file=sys.stderr)if os.path.exists(os.path.join(a.mask_dir,f))for it,sz in[(load_image_to_tensor(os.path.join(a.input_dir,f),norm=False))]for it_n in[it*2-1]for mt in[load_mask_to_tensor(os.path.join(a.mask_dir,f))]for it_n,mt in[(it_n.to(d),mt.to(d))]for o in[m(torch.cat((it_n,mt),1),mt)]];print(json.dumps({'processed_frame_folder':os.path.abspath(a.output_dir)}))"),
