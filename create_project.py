...
def run_command(command):
    ...
    try:
        ...
        if "error" in result: sys.exit(f"[Lỗi Module] {result.get('error')}")
        return result
    except Exception as e: sys.exit(f"[Lỗi Script] Lệnh thất bại: {e}")
...
if __name__ == '__main__': main()"""
    ...
    ("tool/get_file_type.py", "import os, argparse, json, sys; p=argparse.ArgumentParser(); p.add_argument('--input',required=True); a=p.parse_args(); v=['.mp4','.avi','.mov']; i=['.jpg','.jpeg','.png']; e=os.path.splitext(a.input)[1].lower(); print(json.dumps({'file_type':'video' if e in v else 'image' if e in i else 'unknown'}))"),
    ...
    ("tool/duration_split.py", "import argparse, json, sys, os, psutil; from ffmpy import FFmpeg; p=argparse.ArgumentParser(); p.add_argument('--input',required=True); p.add_argument('--fps',type=float,required=True); a=p.parse_args(); t=os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'tmp', 'tf.png'); os.makedirs(os.path.dirname(t), exist_ok=True); ff=FFmpeg(global_options=['-y'],inputs={a.input:['-ss','00:00:01']},outputs={t:['-vframes','1']}); d=300; try: ff.run(stderr=sys.stderr); dr=os.path.getsize(t)*3*a.fps; us=psutil.disk_usage('.').free*0.8; d=int(us/dr) if dr>0 else 300; finally: (lambda:os.remove(t)) [os.path.exists(t)](); print(json.dumps({'optimal_chunk_duration':d}))"),
...
def main():
    ...
    model_path = os.path.join(os.path.dirname(os.path.dirname(SCRIPT_DIR)), 'pre_trained_models', 'add_youknow.pth')
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--input_dir',required=True);p.add_argument('--output_dir',required=True);a=p.parse_args(); dev='cuda' if torch.cuda.is_available() else 'cpu'
    model_path = os.path.join(os.path.dirname(os.path.dirname(SCRIPT_DIR)), 'pre_trained_models', 'mosaic_position.pth')
...
def main():
    ...
    model_path = os.path.join(os.path.dirname(os.path.dirname(SCRIPT_DIR)), 'pre_trained_models', 'clean_youknow.pth')
